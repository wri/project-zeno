...
def convert_date_range_to_days(date_range):
    ...
    return (
        (start_date - REFERENCE_DATE).days,
        (end_date - REFERENCE_DATE).days,
        start_date,
        end_date,
    )
...
def load_natural_lands_mosaic(aoi_geom, start_date, end_date):
    ...
    da = stack(
        items, bounds_latlon=aoi_geom.bounds, snap_bounds=True, epsg=4326
    ).chunk({"x": 1024, "y": 1024})
    return (
        da.astype("int16").max("time").squeeze()
        if "time" in da.dims
        else da.squeeze()
    )
...
def summarize_zonal_alerts(input_text: str) -> Dict:
    ...
    days_since_2015[days_since_2015 == 9999] = days_since_2015[
        days_since_2015 < 9999
    ].max()

    land_cover = load_natural_lands_mosaic(
        shape(parsed.aoi["geometry"]), start_date, end_date
    )
    if land_cover.shape != confidence.shape:
        raise ValueError(
            f"Shape mismatch: {land_cover.shape} vs {confidence.shape}"
        )
