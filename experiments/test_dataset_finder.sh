...
FOLLOWUP_QUERY="If I have to pick one for my analysis which one should it be?"
RUN_FOLLOWUP=true

# Display usage information if --help is provided
if [ "$1" = "--help" ]; then
  echo "Usage: ./test_dataset_finder.sh [query] [persona] [thread_id] [url] [followup_query] [run_followup]"
