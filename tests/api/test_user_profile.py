...
class TestProfileConfigAPI:
    ...
    async def test_get_profile_config(self, client):
        ...
        assert "gis_expertise_levels" in data and len(data["gis_expertise_levels"]) > 0
...
class TestUserProfileAPI:
    ...
    async def test_update_profile_requires_auth(self, client):
        ...
        response = await client.patch("/api/auth/profile", json={"first_name": "John"})
        assert response.status_code == 401
        assert "Missing Bearer token" in response.json()["detail"]

    @pytest.mark.asyncio
    async def test_update_profile_success_basic_fields(self, client, user, auth_override):
        ...
        assert data["profileDescription"] == "Forest researcher interested in conservation"
        assert data["id"] == user.id
        assert data["name"] == user.name  # Original fields preserved

    @pytest.mark.asyncio
    async def test_update_profile_success_detailed_fields(self, client, user, auth_override):
        ...
        assert data["areasOfInterest"] == "Deforestation, Biodiversity, Climate Change"

    @pytest.mark.asyncio
    async def test_update_profile_partial_update(self, client, user, auth_override):
        """Test partial profile updates only change specified fields."""
        auth_override(user.id)
        
        # First update
        response = await client.patch("/api/auth/profile", json={"first_name": "John", "job_title": "Analyst"})
        assert response.status_code == 200
        data = response.json()
        assert data["firstName"] == "John"
        ...
        response = await client.patch("/api/auth/profile", json={"last_name": "Doe"})
        assert response.status_code == 200
        data = response.json()
    ...
    @pytest.mark.asyncio
    async def test_update_profile_empty_update(self, client, user, auth_override):
    ...
    @pytest.mark.asyncio
    async def test_update_profile_validation_errors(self, client, user, auth_override):
        """Test validation errors for invalid field values."""
        auth_override(user.id)
        
        # Test invalid sector
        response = await client.patch("/api/auth/profile", json={"sector_code": "invalid_sector"})
        assert response.status_code == 422
        assert "Invalid sector code" in str(response.json())
        
        # Test invalid country
        response = await client.patch("/api/auth/profile", json={"country_code": "XX"})
        assert response.status_code == 422
        assert "Invalid country code" in str(response.json())
        
        # Test invalid language
        response = await client.patch("/api/auth/profile", json={"preferred_language_code": "xx"})
        assert response.status_code == 422
        assert "Invalid language code" in str(response.json())
        
        # Test invalid expertise level
        response = await client.patch("/api/auth/profile", json={"gis_expertise_level": "invalid"})
        assert response.status_code == 422
    ...
    async def test_user_auto_creation(self, client, auth_override):
        ...
        response = await client.patch("/api/auth/profile", json={"first_name": "Alice"})
        assert response.status_code == 200
...
class TestProfileConfigsStructure:
    ...
    def test_configs_exist_and_valid(self):
        ...
        assert isinstance(GIS_EXPERTISE_LEVELS, dict) and len(GIS_EXPERTISE_LEVELS) > 0