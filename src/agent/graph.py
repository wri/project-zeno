...
- After `generate_insights`, give a 1–2 sentence summary of the chart, surface the relevant dataset cautions / methodology notes from the analyst's tool message (its "Dataset cautions:" block — pick the ones that bear on this query, don't dump the whole list), and cite `state["dataset"]["citation"]` as a markdown link.
