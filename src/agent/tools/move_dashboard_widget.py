...
of its new container. Widget ids come from `inspect_view_context`, which
lists every widget with its id. Owner-only, like the other primitives.
"""
...
async def move_dashboard_widget(
    ...
    widget goes to the end. Only dashboards the user owns can be edited.
    """
    ...
    if section:
        ...
        if target_section.id == widget.section_id:
            return error_command(
                f"{_describe(widget)} is already in section "
                f"'{target_section.title}' — nothing to move. Pass a "
                "position if you meant to reorder it.",
                tool_call_id,
            )
    elif widget.section_id is None:
        return error_command(
...
SPEC = ToolSpec(
    ...
    prompt_fragment=(
        ...
        "(`ungroup=True`) — exactly one of the two. Widget ids come from "
        "inspect_view_context. Use when the user asks to move, regroup or "
        "reorder existing dashboard content."
    ),
)
