...
class ChartInsight(BaseModel):
    """
    Rendering configuration and display text for a chart.
    Title is generated by GEMINI_FLASH which has full access to the query,
    analysis output, and chart data — then passed to the agent so its chat
    response stays anchored to the same title.
    """

    title: str = Field(
        description=(
            "Write what this chart helps the user understand: the metric, place, and time. "
            "Use the user's question to choose framing (comparison vs. trend vs. total). "
            "State facts only, no interpretation. Max 80 characters."
        )
    )
    ...
    follow_up_suggestions: List[str] = Field(
        description="List of 1-2 follow-up suggestions based on available data & capability. Used to guide the agent's response."
    )
...
async def generate_insights(
    ...
    chart_insight_prompt = f"""Generate chart rendering configuration from the analysis output below.
Do NOT generate a title or insight text — those will be written by the agent in the chat response.
...
"""
    ...
    # 7. BUILD RESPONSE
    # Pass the raw analysis output and data summary to the agent so it can write
    # a grounded chat message. The agent also generates the chart title as the
    # first line of its response ("Title: ..."), which the streaming layer extracts
    # and patches into charts_data — keeping title and chat text in sync.
    truncated_output = text_output[:3000] if len(text_output) > 3000 else text_output
    data_summary = (
        chart_data_df.describe().to_string()
        if len(chart_data_df) > 0
        else "No data available"
    )

    tool_message = f"## Chart Title\n{chart_insight_response.title}"
    tool_message += "\n\n## Analysis Output\n"
    tool_message += truncated_output
    tool_message += "\n\n## Chart Data Summary\n"
    tool_message += data_summary
    ...
    for i, suggestion in enumerate(
        ...
        tool_message += f"\n{i}. {suggestion}"
    tool_message += (
        "\n\nUse the Chart Title above verbatim when referencing the chart. "
        "Cite specific numbers from the Analysis Output in your response."
    )
