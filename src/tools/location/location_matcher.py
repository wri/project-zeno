...
class LocationMatcher:
    ...
    def find_matches(self, query, threshold=70):
        ...
        if len(query_parts) > 1:
            compound_matches = self._find_compound_matches(query_parts, threshold)
    ...
    def _find_compound_matches(self, query_parts, threshold):
        ...
        for idx, row in self.df.iterrows():
            ...
            if part_matches:
                avg_score = sum(score for score, _ in part_matches) / len(part_matches)
                if avg_score >= threshold:
                    # Determine match type based on highest priority level matched
                    match_type = 'adm2' if 'adm2' in matched_levels else \
                               'adm1' if 'adm1' in matched_levels else 'iso'
                    matches.extend(self._format_results(
                        self.df.iloc[[idx]],
                        avg_score,
                        match_type
                    ))
...
if __name__ == "__main__":
    ...
    for query in test_queries:
        ...
        for match in matches:
            print(f"Match (score: {match['score']}, type: {match['match_type']}):")
