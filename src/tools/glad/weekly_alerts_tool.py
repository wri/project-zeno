...
class GladAlertsInput(BaseModel):
    """Input schema for GLAD alerts tool"""
    iso: str = Field(description="ISO 3-letter country code (e.g., 'MDG' for Madagascar)")
    adm1: str = Field(description="Administrative level 1 ID")
    adm2: str = Field(description="Administrative level 2 ID")
    year: int = Field(
        description="Year for which to fetch alerts",
        ge=2015,
        le=2024
    )
...
def get_glad_alerts(
    ...
    """Fetch GLAD alerts for a specific administrative region and time period."""
    ...
    try:
        ...
        if not data.get('data'):
            return f"No GLAD alerts found for the specified region and time period:\n" \
