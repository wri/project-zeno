...
def export_tables_to_parquet(db_path: str, output_dir: str):
    ...
    for table, filename in tables_to_export.items():
        ...
        try:
            ...
            conn.sql(f"COPY ({export_query}) TO '{output_path}' (FORMAT PARQUET)")
    ...
    try:
        conn.sql(f"COPY (SELECT * FROM geometries) TO '{geometry_path}' (FORMAT PARQUET)")
...
def main():
    parser = argparse.ArgumentParser(description="Export optimized parquet files")
    parser.add_argument(
        "--db-path",
